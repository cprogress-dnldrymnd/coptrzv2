...
/**
 * @snippet       Disable Add to Cart Except for One Category - WooCommerce
 * @author        Gemini
...
add_filter('woocommerce_is_purchasable', 'woocommerce_is_purchasable_except_one_category', 10, 2);

function woocommerce_is_purchasable_except_one_category($is_purchasable, $product)
{
	// --- CONFIGURATION: SET YOUR ALLOWED CATEGORY SLUG HERE ---
	$allowed_category_slug = 'training';

	// If the product is in the allowed category, it remains purchasable.
	if (has_term($allowed_category_slug, 'product_cat', $product->get_id())) {
		return true;
	}
...
add_filter('woocommerce_add_to_cart_validation', 'block_add_to_cart_except_one_category', 10, 3);

function block_add_to_cart_except_one_category($passed, $product_id, $quantity)
{
	// --- CONFIGURATION: SET YOUR ALLOWED CATEGORY SLUG HERE (must match above) ---
	$allowed_category_slug = 'training';

	// Check if the product is in the allowed category. If it is, validation passes.
	if (has_term($allowed_category_slug, 'product_cat', $product_id)) {
		return true; // true means validation passed
	} else {
		// If not in the allowed category, block it and show an error message.
